        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.monitor_mode != 'off' : # Then there is a metric to monitor
                if self.monitor_metric in log: # Then we have measured it in this epoch
                    ...
                    is_improvement = ((self.monitor_mode == 'min' and current_value < self.monitor_best) or
                        (self.monitor_mode == 'max' and current_value > self.monitor_best)
                    )
                    if is_improvement:
                        self.monitor_best = current_value
                        self.not_improved_count = 0
                        best_model_path = os.path.join(self.checkpoint_dir, f'best_model.pth')
                        self.save_model(path=best_model_path)
                        self.logger.info(f"Epoch {self.current_epoch}: Best model saved with {self.monitor_metric} = {current_value:.5f}")
                    else:
                    ...
                    if self.early_stop > 0:
                        if self.not_improved_count >= self.early_stop:
                            self.logger.info(f"Validation performance didn't improve for {self.early_stop} epochs. Training stops early at epoch {self.current_epoch}.")
                            break
                    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                else:
